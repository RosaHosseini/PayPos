    ...
    fun `onLoadData would navigate to LoadingDestination when receive PROCESSING`() =
        coroutineTestCase {
            given {
                ...
                ) doReturn flowOf(mockedTransaction(State.PROCESSING))
            }
            whenever {
            ...
            then {
                ...
                TestCase.assertEquals(
                    viewModel.transactionUiState.value,
    ...
    fun `onLoadData would navigateTo TransactionFailureDestination when receive FAILURE`() =
        coroutineTestCase {
            given {
                whenever(
                    transactionManager.newTransactionFlow()
                ) doReturn flowOf(mockedTransaction(State.FAILURE))
            }
            whenever {
                viewModel // init viewModel
            }
    ...
    fun `onLoadData would navigateTo TransactionSucceedDestination when receive SUCCESS`() =
        coroutineTestCase {
            given {
                whenever(
                    transactionManager.newTransactionFlow()
                ) doReturn flowOf(mockedTransaction(State.SUCCESS))
            }
            whenever {
                viewModel // init viewModel
            }
    ...
    fun `onLoadData would navigateTo AwaitingAmountDestination when receive AWAITING_AMOUNT`() =
        coroutineTestCase {
            given {
                whenever(
                    transactionManager.newTransactionFlow()
                ) doReturn flowOf(mockedTransaction(State.AWAITING_AMOUNT))
            }
            whenever {
                viewModel // init viewModel
            }
    ...
    fun `onLoadData navigateTo AwaitingConfirmationDestination when get AWAITING_CONFIRMATION`() =
        coroutineTestCase {
            given {
                whenever(
                    transactionManager.newTransactionFlow()
                ) doReturn flowOf(mockedTransaction(State.AWAITING_CONFIRMATION))
            }
            whenever {
                viewModel // init viewModel
            }
